...
- Runtime config is `config.json` inside the Moltpy data directory (default: `~/.moltpy/config.json`); reload via TUI command `reload`. See `src/core/bootstrap/Runtime.py`.
...
## Logging & UI
- Logging uses `MoltpyLogger` with Rich-aware sinks; file logging is controlled by `logging.*` config keys and rotation happens in `MoltpyRuntime.configure_logging()`. See `src/core/Logger.py` and `src/core/bootstrap/Runtime.py`.
...
## Developer Workflows
- Install deps: `python -m venv .venv` then `.\.venv\Scripts\Activate.ps1` then `pip install -r requirements.txt`. See `docs/setup/installation.md`.
- Run locally: `python src\moltpy.py`. See `docs/setup/quickstart.md`.
